...
async function seedGodAccount() {
  ...
  const { STARTING_MONEY, MAX_LEVEL } = require("./gameConfig");
  ...
  await client.execute({
    ...
    args: [uuidv4(), godUsername, hash, MAX_LEVEL + 1, STARTING_MONEY["THE GOD"], JSON.stringify(loadout.skills), JSON.stringify(loadout.items), Date.now()],
  });
