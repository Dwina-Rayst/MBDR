...
async function authMiddleware(req, res, next) {
  ...
  try {
    ...
    if (row.is_god) {
      // THE GOD 계정은 항상 고정 레벨(GOD_LEVEL) + gameConfig.js에 있는 모든 스킬/아이템을 자동으로 보유하도록 보정
      ...
      const needsUpdate = row.level !== GOD_LEVEL || row.skills !== allSkills || row.items !== allItems;
      if (needsUpdate) {
        await client.execute({
          sql: "UPDATE users SET level = ?, rank = 'THE GOD', skills = ?, items = ? WHERE id = ?",
          args: [GOD_LEVEL, allSkills, allItems, row.id],
        });
        row = { ...row, level: GOD_LEVEL, rank: "THE GOD", skills: allSkills, items: allItems };
      }
    }
