...
async function authMiddleware(req, res, next) {
  ...
  try {
    ...
    // THE GOD 계정은 MAX_LEVEL이 바뀔 때마다 자동으로 최고 레벨을 유지하도록 보정
    if (row.is_god && row.level !== MAX_LEVEL + 1) {
      await client.execute({ sql: "UPDATE users SET level = ?, rank = 'THE GOD' WHERE id = ?", args: [MAX_LEVEL + 1, row.id] });
      row = { ...row, level: MAX_LEVEL + 1, rank: "THE GOD" };
    }
